...
KNN_NEIGHBORS = 5     # число соседей для KNN
...
def segment_hand(frame):
    ...
    # mask_h = cv2.inRange(h, int(lower_hsv[0]), int(upper_hsv[0]))
    # mask_s = cv2.inRange(s, int(lower_hsv[1]), int(upper_hsv[1]))
    # mask_v = cv2.inRange(v, int(lower_hsv[2]), int(upper_hsv[2]))
    # cv2.imshow("Hue mask", mask_h)
    # cv2.imshow("Saturation mask", mask_s)
    # cv2.imshow("Value mask", mask_v)
...
def realtime_recognition():
    ...
    while True:
        ...
        cv2.imshow("Real-time HOG+KNN", frame)
